...
public class TransformService {
    ...
    public List<StationObservationsDTO> transform(Collection<MetObsSampleData> data) {
        ...
        Map<String, StationObservationsDTO> stationIdToStation = new HashMap<>();
        for (var sample : data) {
            var observation = transformToObservation(sample);
            if (observation == null) {
                continue;
            }
            var stationId = sample.getStation().getKey();
            var stationObservations = stationIdToStation.get(stationId);
            if (stationObservations == null) {
                var observationList = new ArrayList<ObservationsDTO>();
                stationObservations = new StationObservationsDTO(
                    null,
                    sample.getStation().getName(),
                    stationId,
                    observationList);
                stationIdToStation.put(stationId, stationObservations);
            }
            stationObservations.observations().add(observation);
        }

        var stationList = new ArrayList<>(stationIdToStation.values());
        return stationList;
    }

    public ObservationsDTO transformToObservation(MetObsSampleData sampleData) {
